...
export default function Activities() {
  ...
  const load = () => fetch(apiUrl).then((response) => { if (!response.ok) throw new Error('Unable to load activities'); return response.json() }).then((payload) => setItems(getItems(payload))).catch((requestError) => setError(requestError.message)).finally(() => setLoading(false))
  useEffect(() => { load() }, [])
