...
export default function Teams() {
  ...
  useEffect(() => { fetch(apiUrl).then((response) => { if (!response.ok) throw new Error('Unable to load teams'); return response.json() }).then((payload) => setItems(Array.isArray(payload) ? payload : payload?.results || payload?.data || payload?.items || [])).catch((requestError) => setError(requestError.message)).finally(() => setLoading(false)) }, [])
