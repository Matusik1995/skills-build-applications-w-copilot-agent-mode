...
export default function Users() {
  ...
  useEffect(() => { fetch(`${apiRoot}/api/users/`).then((response) => { if (!response.ok) throw new Error('Unable to load users'); return response.json() }).then((payload) => setItems(Array.isArray(payload) ? payload : payload?.results || payload?.data || payload?.items || [])).catch((requestError) => setError(requestError.message)).finally(() => setLoading(false)) }, [])
